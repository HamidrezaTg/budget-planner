...
const PAGES = [
  ...
  },
  {
    ...
    items: [
      ...
      ['Android app', 'A pure client: on first launch it asks for the server address, verifies the Budget Planner discovery endpoint, and remembers up to ten servers. If the server is unreachable, the recovery screen offers \u201cTry again\u201d and \u201cChange server address\u201d.'],
      ['Switch the server (Android)', 'Use the server switch action in the app or press the system Back button. The address screen lists saved servers; choose one, enter a new address, or forget an old one.'],
      ['Tailscale', 'Install Tailscale on the server and phone, then enter the server’s Tailscale IP or MagicDNS hostname plus the planner port, for example http://100.x.y.z:2026. The server must bind to that interface.'],
  ...
  },
  {
    ...
    items: [      ['Provider / API key / Model', 'Choose an AI provider, enter your key, load the models available to your key, pick one. Stored in your own database only.'],
      ...
      ['Privacy mode', 'Use the eye button in the sidebar to blur financial values while keeping navigation and labels visible. The choice is remembered in this browser.'],
    ],
  },
  {
    title: 'Categories',
