...
export default function Categories() {
  ...
  const [addCat, setAddCat] = useState({ name: '', group_id: '', account_id: '', monthly_budget: 0 });
  ...
  const removeCategory = async (c) => {
    const ok = await confirm({
      ...
      message: 'If any transactions are still tagged with it, deletion is refused — retire the category instead so the history is kept.',
      danger: true,
  ...
  const saveEditCategory = async (c) => {
    ...
    if (!e) return;
    if (!e.name.trim()) { setError('Name is required.'); return; }
    try {
  ...
  const cancelRenameGroup = (id) =>
    setRenamingGroup((p) => { const n = { ...p }; delete n[id]; return n; });
  const saveRenameGroup = async (g) => {
    const v = (renamingGroup[g.id] ?? '').trim();
    if (!v || v === g.name) { cancelRenameGroup(g.id); return; }
    try {
  ...
  return (
    <div>
      <h1>Categories</h1>
      <p className="muted">
        Every category belongs to exactly one group (budget block) and one account.
        Retiring a category clears its plan and rules so nothing phantom survives.
      </p>
      ...
      <div className="card table-card">
        ...
        <table>
          <thead><tr><th>Name</th><th>Sort</th><th>Categories</th><th></th></tr></thead>
          <tbody>
            {meta.groups.length === 0 && (
              <tr><td colSpan={4} className="muted">No groups yet — create one to organize your categories.</td></tr>
            )}
            {meta.groups.map((g) => {
              ...
              return (
                <tr key={g.id}>
                  <td>
                    {renamingGroup[g.id] !== undefined ? (
                      <input
                        ...
                        onChange={(e) => setRenamingGroup((p) => ({ ...p, [g.id]: e.target.value }))}
                        onKeyDown={(e) => {
                  ...
                  <td>
                    {renamingGroup[g.id] !== undefined ? (
                      <>
                        <button className="btn small primary" onClick={() => saveRenameGroup(g)} title="Save the new group name">Save</button>
                        <button className="btn ghost small" onClick={() => cancelRenameGroup(g.id)} title="Cancel without renaming">Cancel</button>
                      </>
                    ) : (
                      <>
                        <button
      ...
      <div className="card table-card" style={{ marginTop: 14 }}>
        ...
        {showAddCat && (
          <form onSubmit={submitAddCategory} className="add-cat-form">
            ...
            <input
              type="number" step="0.01" min="0" placeholder="€/month"
        ...
        <table>
          <thead>
            <tr>
              <th>Category</th><th>Group</th><th>Account</th>
              <th className="num">Plan €/mo</th><th>Status</th><th></th>
            </tr>
          </thead>
          <tbody>
            {cats.map((c) => {
                ...
                <tr key={c.id} className={c.is_active ? '' : 'done-row'}>
                  <td>
                    {edit ? (
                      <input
                        autoFocus
                        value={edit.name}
                        onChange={(e) => setEditingCat((p) => ({ ...p, [c.id]: { ...p[c.id], name: e.target.value } }))}
                        maxLength={60}
                      />
                    ) : c.name}
                  </td>
                  <td>
                    {edit ? (
                      <select
                        value={edit.group_id}
                        onChange={(e) => setEditingCat((p) => ({ ...p, [c.id]: { ...p[c.id], group_id: e.target.value } }))}
                      >
                        <option value="">No group</option>
                        {meta.groups.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
                      </select>
                  ...
                  <td>
                    {edit ? (
                      <select
                        ...
                        onChange={(e) => setEditingCat((p) => ({ ...p, [c.id]: { ...p[c.id], account_id: e.target.value } }))}
                      >
                        <option value="">— untagged —</option>
                        {meta.accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
                      </select>
                  ...
                  <td className="num">
                    {edit ? (
                      <input
                        ...
                        onChange={(e) => setEditingCat((p) => ({ ...p, [c.id]: { ...p[c.id], monthly_budget: e.target.value } }))}
                      />
                    ) : (
                  ...
                  <td>
                    {edit ? (
                      <label className="muted tiny" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                        <input
                          type="checkbox"
                          checked={!!edit.is_active}
                          onChange={(e) => setEditingCat((p) => ({ ...p, [c.id]: { ...p[c.id], is_active: e.target.checked } }))}
                        /> active
                      </label>
                    ) : c.is_active ? (
                      <button
                        ...
                        onClick={() => patch(c.id, { is_active: false }, {
                          ...
                          message: 'Retiring clears its monthly plan and categorization rules so nothing phantom survives. You can reactivate it later.',
                          danger: true,
                  ...
                  <td>
                    {edit ? (
                      <>
                        <button className="btn small primary" title="Save the changes" onClick={() => saveEditCategory(c)}>Save</button>
                        <button className="btn ghost small" title="Cancel without saving" onClick={() => cancelEditCategory(c.id)}>Cancel</button>
                      </>
                    ) : (
                      <>
                        <button
      ...
      <form onSubmit={addRule} className="card inline-form rule-form">
        ...
        <button className="btn primary" title="Add this keyword rule">Add rule</button>
      </form>
      ...
      <form onSubmit={addAdvancedRule} className="card inline-form rule-form">
        <input title="Match if description contains this text" placeholder="Description contains" value={advancedRule.description_contains}
          onChange={(e) => setAdvancedRule({ ...advancedRule, description_contains: e.target.value })} />
        <input type="number" step="0.01" title="Match if the absolute amount is at least this" placeholder="Min €" value={advancedRule.amount_min}
          onChange={(e) => setAdvancedRule({ ...advancedRule, amount_min: e.target.value })} />
        <input type="number" step="0.01" title="Match if the absolute amount is at most this" placeholder="Max €" value={advancedRule.amount_max}
          onChange={(e) => setAdvancedRule({ ...advancedRule, amount_max: e.target.value })} />
        <select title="Match only transactions on this account" value={advancedRule.account_id} onChange={(e) => setAdvancedRule({ ...advancedRule, account_id: e.target.value })}>
          <option value="">Any account</option>
          {meta.accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
        </select>
        <input title="Match the tx_type (e.g. Card)" placeholder="Type (e.g. Card)" value={advancedRule.tx_type}
          onChange={(e) => setAdvancedRule({ ...advancedRule, tx_type: e.target.value })} />
        <select title="Category to apply when the rule matches" value={advancedRule.category_id} onChange={(e) => setAdvancedRule({ ...advancedRule, category_id: e.target.value })}>
          <option value="">→ Category</option>
        ...
        <button className="btn primary" title="Add this advanced rule">Add advanced rule</button>
      </form>
      ...
      <form onSubmit={testRule} className="card inline-form rule-form">
        <input title="A sample transaction description" placeholder="Description" value={testTx.description}
          onChange={(e) => setTestTx({ ...testTx, description: e.target.value })} />
        <input type="number" step="0.01" title="Signed amount (negative = spend)" placeholder="Amount €" value={testTx.amount}
          onChange={(e) => setTestTx({ ...testTx, amount: e.target.value })} />
        <select title="Account the sample transaction is on" value={testTx.account_id} onChange={(e) => setTestTx({ ...testTx, account_id: e.target.value })}>
          <option value="">Any account</option>
          {meta.accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
        </select>
        <input title="tx_type of the sample transaction" placeholder="Type" value={testTx.tx_type}
          onChange={(e) => setTestTx({ ...testTx, tx_type: e.target.value })} />
        <button className="btn" title="Find the rule that would match this transaction">Test</button>
