...
test('deleting a split parent cascades to its children', () => {
  ...
  const parent = db
    .prepare(
      `INSERT INTO transactions (date, description, amount, currency, account_id, category_id, dedup_key, split_group)
              VALUES ('2026-05-01', 'Shop', -100, 'EUR', ?, ?, 'split-x', 'grp1')`,
    )
    .run(acc, cat).lastInsertRowid;
  db.prepare(
    `INSERT INTO transactions (date, description, amount, currency, account_id, category_id, dedup_key, split_group, split_of)
              VALUES ('2026-05-01', 'Shop', -60, 'EUR', ?, ?, 'split-y', 'grp1', ?)`,
  ).run(acc, cat, parent);
  db.prepare(
    `INSERT INTO transactions (date, description, amount, currency, account_id, category_id, dedup_key, split_group, split_of)
              VALUES ('2026-05-01', 'Shop', -40, 'EUR', ?, ?, 'split-z', 'grp1', ?)`,
  ).run(acc, cat, parent);

  assert.equal(
    db.prepare('SELECT COUNT(*) c FROM transactions WHERE split_of = ?').get(parent).c,
    2,
  );
...
test('foreign keys reject references to missing rows', () => {
  ...
  assert.throws(() => {
    db.prepare(
      "INSERT INTO transactions (date, description, amount, currency, dedup_key, category_id) VALUES ('2026-05-01','X',-1,'EUR','fk-c', 999999)",
    ).run();
...
test('dedup keys are migrated to a currency-aware format once', () => {
  ...
  raw
    .prepare(
      `INSERT INTO transactions (date, description, amount, currency, category_id, dedup_key)
               VALUES ('2026-05-01','Coffee',-4.5,'EUR',?,'2026-05-01|-4.50|coffee')`,
    )
    .run(cat);
  raw
    .prepare(
      `INSERT INTO transactions (date, description, amount, currency, category_id, dedup_key)
               VALUES ('2026-05-02','Coffee',-4.5,'USD',?,'2026-05-02|-4.50|coffee')`,
    )
    .run(cat);
  raw
    .prepare(
      `INSERT INTO transactions (date, description, amount, currency, category_id, dedup_key)
               VALUES ('2026-05-03','Recurring',-10,'EUR',?,'rec|7|2026-05')`,
    )
    .run(cat);
  raw
    .prepare(
      `INSERT INTO transactions (date, description, amount, currency, category_id, dedup_key)
               VALUES ('2026-05-04','Split part',-5,'EUR',?,'split|7|0')`,
    )
    .run(cat);
  raw.close();
  ...
  const keys = db
    .prepare('SELECT dedup_key FROM transactions ORDER BY id')
    .all()
    .map((r) => r.dedup_key);
  ...
  assert.equal(
    db
      .prepare(
        "SELECT COUNT(*) c FROM transactions WHERE dedup_key = '2026-05-01|-4.50|EUR|coffee'",
      )
      .get().c,
    1,
  );
...
test('usernames that collided under the legacy sanitizer get distinct databases', () => {
  ...
  dbm.als.run(plain, () => {
    const hasMark = plain
      .prepare("SELECT COUNT(*) c FROM categories WHERE name = 'DotUserMark'")
      .get().c;
