...
test('allows a plain SELECT on an allowed table', () => {
  assert.equal(
    validateReadOnlySql('SELECT * FROM transactions'),
...
test('rejects write/DDL statements', () => {
  const reject = (sql) =>
    assert.throws(() => validateReadOnlySql(sql), /Only SELECT|forbidden|single/);
...
test('rejects arbitrary user limits above the cap', () => {
  assert.throws(
    () => validateReadOnlySql('SELECT * FROM transactions LIMIT 999999'),
    /LIMIT may not exceed/,
  );
  assert.equal(
    validateReadOnlySql('SELECT * FROM transactions LIMIT 50'),
...
test('joins are checked too', () => {
  assert.throws(
    () => validateReadOnlySql('SELECT * FROM transactions JOIN settings ON 1=1'),
    /not available/,
  );
  assert.equal(
...
test('self-joins are rejected (synchronous event-loop DoS)', () => {
  assert.throws(
    () =>
      validateReadOnlySql(
        'SELECT count(*) FROM transactions t1 JOIN transactions t2 ON abs(t1.amount)=abs(t2.amount)',
      ),
    /same table twice/,
  );
  assert.throws(
    () =>
      validateReadOnlySql(
        'WITH x AS (SELECT * FROM accounts) SELECT * FROM x JOIN accounts ON 1=1',
      ),
    /same table twice/,
  );
  assert.doesNotThrow(() =>
...
test('a LIMIT inside a subquery does not bound the outer result', () => {
  assert.equal(
    validateReadOnlySql(
      'SELECT * FROM transactions WHERE category_id IN (SELECT id FROM categories LIMIT 5)',
    ),
  ...
  assert.throws(
    () =>
      validateReadOnlySql(
        'SELECT * FROM transactions WHERE category_id IN (SELECT id FROM categories LIMIT 500)',
      ),
    /LIMIT may not exceed/,
  );
});
